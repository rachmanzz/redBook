...
redBook.prototype.where = function(key,data){
	if(/^SELECT [\w\d*]+ FROM /.test(this.getSelect())) this.setQuery('WHERE '+key +' = ' +mysql.escape(data));
	return this;
};

redBook.prototype.andWhere = function(key,data){
	if(/^SELECT [\w\d*]+ FROM [\w\d]+ [\w]+ [\w\d]+ = [\w\d]+/.test(this.getSelect())) this.setQuery(' AND '+key +' = ' +mysql.escape(data));
	return this;
};
redBook.prototype.log=function () {
